...
var _ = Describe("MongoDB Service", func() {
	...
	var appPath = "../assets/cf-mongodb-example-app"
	...
	AssertLifeCycleBehavior := func(planName string) {
		It("can create, bind to, write to, read from, unbind, and destroy a service instance using the "+planName+" plan", func() {
			...
			case <-createServiceStdout.Detect("OK"):
				...
				uri := appUri(appName) + "/entries/foo/bar"
				fmt.Println("Posting to url: ", uri)
				Eventually(runner.Curl("-X", "POST", uri, "-k", "-f"), config.ScaledTimeout(timeout), retryInterval).Should(Exit(0))
				fmt.Println("\n")

				uri = appUri(appName) + "/entries/foo"
				fmt.Println("Getting from url: ", uri)
				Eventually(runner.Curl(uri, "-k"), config.ScaledTimeout(timeout), retryInterval).Should(Say("bar"))
				fmt.Println("\n")
